...
export function analyzeNestingAreas(data: DeviceHistory): AnalyzedLocation[] {
  ...
  const calculatedTotalStopMs = allStops.reduce(
    (sum, stop) => sum + stop.durationMs,
    0,
  );
  ...
  let analyzedLocations: AnalyzedLocation[] = clusters.map((stopCluster) => {
    ...
    const representative_lat =
      stopCluster.reduce((sum, s) => sum + s.lat, 0) / appearances;
    const representative_lng =
      stopCluster.reduce((sum, s) => sum + s.lng, 0) / appearances;

    // Sum up the time spent in this cluster
    const location_time_ms = stopCluster.reduce(
...
export function calculateLocationTimeFractions(
  ...
  const totalHours =
    Math.floor(
      locations.reduce((sum, loc) => sum + loc.location_time_hours, 0) * 100,
    ) / 100;
  ...
  return locations.map((loc) => ({
    ...loc,
    location_time_hours_fraction: Math.floor(
