...
export default defineEventHandler(async (event) => {
  ...
  for (const entry of body.entries) {
    ...
    editSpecificCell(
      worksheet,
      `A${intialRow}`,
      entry.driver.name as unknown as string,
    );
    ...
    editSpecificCell(
      worksheet,
      `B${intialRow}`,
      (entry.name as string)?.toUpperCase(),
    );
    ...
    editSpecificCell(
      worksheet,
      `C${intialRow}`,
      entry.driver.phone as unknown as string,
    );
    ...
    editSpecificCell(
      worksheet,
      `D${intialRow}`,
      body.corporateName.toUpperCase(),
    );
    ...
    editSpecificCell(
      worksheet,
      `E${intialRow}`,
      entry.device_data.created_at?.toString(),
    );
    ...
    if (query.return_full && query.return_full) {
      ...
      editSpecificCell(
        worksheet,
        `K${intialRow}`,
        entry.stop_duration_sec?.toString(),
      );
      ...
      editSpecificCell(
        worksheet,
        `L${intialRow}`,
        entry.moved_timestamp?.toString() ?? "-",
      );
      ...
      editSpecificCell(
        worksheet,
        `M${intialRow}`,
        entry.total_distance?.toString() ?? "0",
      );
      ...
      editSpecificCell(
        worksheet,
        `O${intialRow}`,
        entry.traccar.moved_at?.toString() ?? "-",
      );
      editSpecificCell(
        worksheet,
        `P${intialRow}`,
      ...
      editSpecificCell(
        worksheet,
        `X${intialRow}`,
        entry.traccar.course?.toString() ?? "0",
      );
      editSpecificCell(
        worksheet,
        `Y${intialRow}`,
        entry.traccar.speed?.toString() ?? "0",
      );
    }
...
function deduceTrackerStatus(online: TrackerStatusWrapperName): {
  color: string;
  text: string;
} {
  if (online == "Online") {
